import csv      # подключаем библиотеку для работы с csv-файлами
import os.path  # подключаем библиотеку функций операционной системы

числа = []  # пустой массив для кластеризуемых чисел
k = 4 # предполагаемое количество кластеров

if os.path.exists('cluster_new.csv'): # если файл данных уже существует
    # ------------------ чтение данных из файла ------
    print('Чтение данных ...')
    with open('cluster_new.csv', mode="r") as r_file:  # открываем файл на чтение с псевдонимом r_file
        fr = csv.reader(r_file) # подключаем к файлу читалку с псевдонимом fr
        p = int(next(fr)[0]) # читаем сколько элементов в массиве кластеризуемых чисел
        print(p)
        for j in range(p): # для каждого элемента массива
            числа.append(int(next(fr)[0])) # читаем значение из файла и добавляем в массив
    r_file.close  # закрываем прочитанный файл
    print(числа)
...
# Задание:
# 1. создайте в excel csv-файл содержащий 4 нормально распределённых кластера по 50 значений каждый
